...
**Option 3: Amazon Bedrock (Bearer Token)**
...
Uses [Bedrock API keys](https://aws.amazon.com/blogs/machine-learning/accelerate-ai-development-with-amazon-bedrock-api-keys/) for authentication. The credential proxy injects the bearer token into requests forwarded to the regional Bedrock runtime endpoint (`bedrock-runtime.{region}.amazonaws.com`). Containers never see the real token. When Bedrock is enabled, it takes priority over API key and OAuth options.
